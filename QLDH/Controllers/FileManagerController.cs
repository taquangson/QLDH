...
namespace QLDH.Controllers
{
    public class FileManagerController : Controller
    {
                ...
                try
                {
                    ...
                    var result = directoryProvider
                        ...
                        .Select(f => new
                        {
                            ...
                            ngaycap = f.ChiTiet != null ? f.ChiTiet.NgayTao : DateTime.Now,
                            tentailieu = f.ChiTiet != null ? f.ChiTiet.FileName : ""
                        });
        ...
        public virtual ActionResult Destroy(FileManagerEntry entry)
        {
            ...
            File_NhanVienDAO fdao = new File_NhanVienDAO();
            File_NhanVienModel fmodel = fdao.GetByPath(path);
            if (!string.IsNullOrEmpty(path))
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public virtual ActionResult Upload(string TenTaiLieu, DateTime NgayCap, string path, HttpPostedFileBase file)
        {
            path = NormalizePath(path);
            string ngaycap = NgayCap.ToString("dd/MM/yyyy");
            var fileName = Path.GetFileName(file.FileName);
            File_NhanVienModel fmodel = new File_NhanVienModel();
            fmodel.NgayTao = NgayCap;
            fmodel.FilePath = path + "/" + fileName;
            fmodel.FileName = TenTaiLieu;
            File_NhanVienDAO fdao = new File_NhanVienDAO();
            if (AuthorizeUpload(path, file))
            {
                file.SaveAs(Path.Combine(Server.MapPath(path), fileName));
                fdao.InsertFile(fmodel);
                return Json(new
                {
