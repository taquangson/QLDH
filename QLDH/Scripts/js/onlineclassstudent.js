...
function participantConnected(participant) {
    ...
    $.ajax({
        url: '/User/GetAppUser?UserName=' + participant.identity,
        type: 'GET',
    }).done(function (response) {
        var user = response;
        ...
        if (user.EmployeeType == 0) {
            ...
            $("#ul-list-participant").append("<li class='li-participant'><label style='width:100%;color:#fff;font-size:1.2em;font-weight:500'>" + user.UserName + "</label> <div class='group-combo'> " +
                "<input disabled='disabled' type='checkbox' checked='checked' id='cb-video-" + participant.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-video-" + participant.sid + "'> <i class='fa fa-video-camera'></i></label>"
                + "<input disabled='disabled' type='checkbox' checked='checked' onchange='changeVoice(this,\'" + participant.sid + "\')' id='cb-voice-" + participant.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-voice-" + participant.sid + "'> <i class='fa fa-microphone'></i></label>"
                + "<input disabled='disabled' type='checkbox' checked='checked' onchange='changeChat(this,\'" + participant.sid + "\')' id='cb-chat-" + participant.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-chat-" + participant.sid + "'> <i class='fa fa-comments'></i></label>"
                + "</div></li>");
        } else {
            document.getElementById("remoteTrack").append(div);
    ...
    $.ajax({
        ...
        if (response.status) {
            ...
            if (room.EndTime == null && room.Duration == null) {
                ...
                //var idlop = getParameterByName("ID_Lop");
                //createRoom(idlop);
                $("#txtRunning").text("Buổi học sắp bắt đầu!...Vui lòng đợi....");
                alert("Vui lòng đợi cô giáo bắt đâu");
            }
        } else {
            //var idlop = getParameterByName("ID_Lop");
            //createRoom(idlop);
...
$(document).ready(function () {
    ...
    $.each(participants, function (index, item) {
        $("#ul-list-participant").append("<li class='li-participant'><label style='width:100%;color:#fff;font-size:1.2em;font-weight:500'>" + item.name + "</label> <div class='group-combo'> " +
            "<input disabled='disabled' type='checkbox' checked='checked' id='cb-video-" + item.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-video-" + item.sid + "'> <i class='fa fa-video-camera'></i></label>"
            + "<input disabled='disabled' type='checkbox' checked='checked' onchange='changeVoice(this,\'" + item.sid + "\')' id='cb-voice-" + item.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-voice-" + item.sid + "'> <i class='fa fa-microphone'></i></label>"
            + "<input disabled='disabled' type='checkbox' checked='checked' onchange='changeChat(this,\'" + item.sid + "\')' id='cb-chat-" + item.sid + "' class='km-checkbox'/><label class= 'km-checkbox-label' for='cb-chat-" + item.sid + "'> <i class='fa fa-comments'></i></label>"
            + "</div></li>");
        //const div = document.createElement('div');
        //div.className = "participant-view";
