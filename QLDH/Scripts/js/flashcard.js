...
function openEditWindow() {
    ...
    if (arr.length != 1) {
        ...
        $("#ChuDe").val(selectedItem.ChuDe);
        $("#The").val(selectedItem.The);
        $("#Ten").val(selectedItem.Ten);
        $("#AnhThe").val(selectedItem.AnhThe);
        $("#AmThanh").val(selectedItem.AmThanh);
        //$("#filehinhanh").data("kendoFileUpload").value(selectedItem.AnhThe);
        //$("#fileamthanh").data("kendoFileUpload").value(selectedItem.AmThanh);
    }
}
...
function Luu() {
    ...
    var validthe = SetValidate("The");
    var validchude = SetValidate("ChuDe");
    var validanhthe = SetValidate("AnhThe");
    var valid = validten && validthe && validchude && validanhthe;
    if (valid) {
        var model = {
            ID: $("#ID").val(),
            ChuDe: $("#ChuDe").val(),
        ...
        $.ajax({
            url: '/BaiGiang/CreateOrUpdate_Flashcard',
            //processData: false,
            //contentType: false,
            data: model,
            type: 'POST'
...
function Xoa() {
    ...
    if (bangDuLieu.select().length > 0) {
        openConfirm(dialogRoot, "<b style='line-height:40px;'>Bạn có chắc chắn muốn xóa " + bangDuLieu.select().length + " flashcard?</b>", function () { XoaDuLieu(bangDuLieu); }, function () { });
    } else {
...
function XoaDuLieu(bangDuLieu) {
    ...
    $.ajax({
        url: '/BaiGiang/Delete_Flashcard?ID=' + ids.join(','),
        type: 'POST'
