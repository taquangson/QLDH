...
function TimKiem() {
    $.ajax({
        url: '/DangKy/GetDSDangKyEvent?from=' + kendo.toString($("#TuNgay").data("kendoDatePicker").value(), "yyyy/MM/dd") + " 00:00:00"
...
function updateTrangThai(ID) {
    $.ajax({
        url: '/DangKy/UpdateTrangThai_DangKyEvent?ID=' + ID,
        type: 'GET',
    }).done(function successCallback(response) {
        TimKiem();
    });
